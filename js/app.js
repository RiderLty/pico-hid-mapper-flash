...
/** @type {number|null} 连接时检测到的 flash 实际容量（字节）；未检测/无法检测为 null */
let flashSize = null;
...
/**
 * 检测 flash 实际容量。
 * 移植 picotool 的 guess_flash_size()：flash 读取按芯片实际容量回绕
 * （超出容量的地址被 flash 芯片掩码回起始处），因此从大到小探测
 * 2 的幂偏移，首个与首两页内容不同的偏移 ×2 即为实际容量。
 * 失败/无法判断时返回 null，不抛错。
...
async function detectFlashSize() {
    ...
    if (flashEnd == null) return null;

    const start = target.flashStart();
    const pageSize = target.flashPageSize();
    const windowSize = flashEnd - start;
    const probeSize = 2 * pageSize;

    try {
        await withDefaultTimeout(async () => picoboot.resetInterface(), '重置接口');
        await withDefaultTimeout(async () => connection.exitXip(), '退出XIP');

        const basePages = await withDefaultTimeout(
            async () => connection.flashRead(start, probeSize),
            '读取flash首页'
        );

        // 全 0xFF（空白 flash）或前两页相同：无法判断（与 picotool 一致）
        let allFF = true;
        for (const b of basePages) {
            if (b !== 0xFF) { allFF = false; break; }
        }
        if (allFF) return null;
        if (bytesEqual(basePages.slice(0, pageSize), basePages.slice(pageSize))) return null;

        // 从地址窗口的一半开始向下探测（保持探测地址在 XIP 窗口内）
        for (let size = windowSize / 2; size >= 16 * pageSize; size >>= 1) {
            const pages = await withDefaultTimeout(
                async () => connection.flashRead(start + size, probeSize),
                '探测flash大小'
            );
            if (!bytesEqual(basePages, pages)) {
                const detected = size * 2;
                // 过小的结果不可信（内容异常时误判），视为检测失败
                return detected >= 1024 * 1024 ? detected : null;
            }
        }
        return null;
    } catch (error) {
        console.log(`检测 flash 容量出错：${error.message}`);
        return null;
    }
}
...
function updateDeviceInfo() {
    ...
    const rangeEnd = flashSize != null ? start + flashSize : flashEnd;
    deviceFlashRange.textContent = `0x${start.toString(16)} - ${rangeEnd != null ? `0x${rangeEnd.toString(16)}` : '--'}${flashSize != null ? `（实测 ${formatBytes(flashSize)}）` : ''}`;
...
async function connect() {
    ...
    try {
        ...
        // 连接成功后检测 flash 实际容量（失败不影响连接）
        flashSize = null;
        flashSize = await detectFlashSize();
        if (flashSize != null) {
            logActivity(`检测到 flash 实际容量：${formatBytes(flashSize)}`, 'success');
        } else {
            logActivity('无法检测 flash 实际容量（空白 flash 或读取失败），按地址窗口上限处理', 'warning');
        }
        updateDeviceInfo();
    } catch (e) {
...
async function eraseFlash() {
    ...
    // 优先使用连接时检测到的实际容量；检测失败（如空白 flash）才退回地址窗口上限
    const size = flashSize != null ? flashSize : flashEnd - start;
    if (flashSize == null) {
        logActivity('未能检测 flash 实际容量，将按地址窗口上限擦除', 'warning');
    }
